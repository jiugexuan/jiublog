...
**Setup.** The detector is Faster R-CNN [52] with a backbone of R50-dilated-C5 or R50-C4 [32] (details in appendix), with BN tuned, implemented in [60]. We fine-tune all layers end-to-end. The image scale is [480, 800] pixels during training and 800 at inference. The same setup is used for all entries, including the supervised pre-training baseline. We evaluate the default VOC metric of AP$_{50}$ ($i.e.,$ IoU threshold is $50\%$) and the more stringent metrics of COCO-style AP and AP$_{75}$. Evaluation is on the VOC `test2007` set.

**Ablation: backbones.** Table 2 shows the results fine-tuned on `trainval07+12` (∼16.5k images). For R50-dilatedC5 (Table 2a), MoCo pre-trained on IN-1M is comparable to the supervised pre-training counterpart, and MoCo pre-trained on IG-1B *surpasses* it. For R50-C4 (Table 2b),MoCo with IN-1M or IG-1B is *better* than the supervised counterpart: up to $\mathbf{+0.9}$ AP$_{50}$, $\mathbf{+3.7}$ AP, and $\mathbf{+4.9}$ AP$_{75}$.

**消融：主干。** 表 2 显示了在“trainval07+12”（~16.5k 图像）上微调的结果。 对于 R50-dilatedC5（表 2a），在 IN-1M 上预训练的 MoCo 与有监督的预训练对应物相当，并且在 IG-1B 上预训练的 MoCo *超过*它。 对于 R50-C4（表 2b），带有 IN-1M 或 IG-1B 的 MoCo 比有监督的对应物*更好*：高达 $\mathbf{+0.9}$ AP$_{50}$, $\mathbf{+ 3.7}$ AP 和 $\mathbf{+4.9}$ AP$_{75}$。
